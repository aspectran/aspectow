 ...
 */
package com.aspectran.aspectow.console.common.pagination;
...
public class PageInfo {
    ...
    @NonNull
    public static PageInfo of(@NonNull Translet translet, String cookieName) {
        return of(translet, DEFAULT_PAGE_SIZE, cookieName);
    }

    @NonNull
    public static PageInfo of(@NonNull Translet translet, int defaultPageSize, String cookieName) {
        String pageNumber = translet.getParameter("page");
        String pageSize = translet.getParameter("size");
        int number = StringUtils.isEmpty(pageNumber) ? 1 : Integer.parseInt(pageNumber);
        ...
        } else if (StringUtils.hasText(cookieName)) {
            try {
